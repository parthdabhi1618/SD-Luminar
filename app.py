...
def check_conversion_status(filename):
    status = conversion_status.get(filename, {'status': 'unknown'})
...
def upload_and_analyze():
    ...
    # Avoid clobbering by adding uuid if file exists
...
def get_page_count():
    ...
    # filename may be a full path or relative
    try:
        path = filename
        ...
        return jsonify({'pageCount': 0}), 500
