...
def upload_and_analyze():
    ...
    file.save(filepath)
    stats = get_doc_stats(filepath) if ext == '.pdf' else None
    return jsonify({'serverFilename': server_filename, 'initialStats': stats})
...
def extract_highlights_route():
    ...
    try:
        ...
        pdf_filename = server_filename.replace('.pdf', '_notes.pdf')
        pdf_path = os.path.join(app.config['UPLOAD_FOLDER'], pdf_filename)
        create_modern_pdf(highlights, pdf_path)
        final_stats = get_doc_stats(pdf_path)
        docx_filename = server_filename.replace('.pdf', '_notes.docx')
...
def add_header_footer_route():
    ...
    if server_filename.lower().endswith('.ipynb'):
        try:
