...
def _convert_ipynb_to_pdf_async(src_path, timeout=120):
    ...
    try:
        ...
        # Use allow-chromium-download to be explicit; this may still fail if chromium is not available
        subprocess.run(['jupyter', 'nbconvert', '--to', 'webpdf', '--allow-chromium-download', src_path], check=True, timeout=timeout)
        if os.path.exists(pdf_output_path):
            conversion_status[os.path.basename(src_path)] = {'status': 'done', 'pdf_path': pdf_output_path}
            print(f"Background conversion completed: {pdf_output_path}")
        else:
            conversion_status[os.path.basename(src_path)] = {'status': 'failed', 'pdf_path': None}
            print(f"Background conversion finished but output not found for {src_path}")
    except Exception as e:
        conversion_status[os.path.basename(src_path)] = {'status': 'failed', 'pdf_path': None}
...
def get_pdf_for_serverfile(server_filename, input_filepath, wait_seconds=5, sync_timeout=60):
    ...
    """
    # Already a PDF
    if server_filename.lower().endswith('.pdf'):
        return input_filepath

    # Look up conversion status
    ...
    try:
        ...
        if os.path.exists(pdf_output_path):
            conversion_status[server_filename] = {'status': 'done', 'pdf_path': pdf_output_path}
            return pdf_output_path
    except Exception as e:
        print(f"Synchronous nbconvert fallback failed for {input_filepath}: {e}")
        conversion_status[server_filename] = {'status': 'failed', 'pdf_path': None}
...
def upload_and_analyze():
    ...
    try:
        if ext == '.pdf':
            stats = get_doc_stats(filepath)
        elif ext == '.ipynb':
            # Start background conversion to PDF so downstream processing can use the PDF
            conversion_status[server_filename] = {'status': 'pending', 'pdf_path': None}
            # Try a short synchronous convert first to get a quick page count when possible
            pdf_output_path = os.path.splitext(filepath)[0] + '.pdf'
            try:
                subprocess.run(['jupyter', 'nbconvert', '--to', 'webpdf', '--allow-chromium-download', filepath], check=True, timeout=30)
                if os.path.exists(pdf_output_path):
                    stats = get_doc_stats(pdf_output_path)
                    conversion_status[server_filename] = {'status': 'done', 'pdf_path': pdf_output_path}
            except Exception as e:
                # Synchronous conversion failed or timed out; kick off background thread and continue
                print(f"nbconvert quick attempt failed for {filepath}: {e}")
                try:
                    t = threading.Thread(target=_convert_ipynb_to_pdf_async, args=(filepath, 120), daemon=True)
                    t.start()
                except Exception as e2:
                    conversion_status[server_filename] = {'status': 'failed', 'pdf_path': None}
                    print(f"Failed to start background conversion thread for {filepath}: {e2}")

        if stats and isinstance(stats, dict):
            page_count = stats.get('pages') or stats.get('pageCount') or stats.get('numPages')
        # Fallback default
        if not page_count:
            page_count = 1
    except Exception as e:
        print(f"Failed to determine page count for {filepath}: {e}")
        page_count = 1

    conv = conversion_status.get(server_filename)
    converted_pdf_name = None
    conversion_state = None
    if conv:
        conversion_state = conv.get('status')
        if conv.get('pdf_path') and os.path.exists(conv.get('pdf_path')):
            converted_pdf_name = os.path.basename(conv.get('pdf_path'))
    ...
    return jsonify({
        ...
        'pages': page_count,
        'numPages': page_count,
        'conversionStatus': conversion_state,
        'convertedPdf': converted_pdf_name
    })
...
def extract_highlights_route():
    ...
    try:
        ...
        create_modern_pdf(highlights, pdf_path)
        final_stats = get_doc_stats(pdf_path)
